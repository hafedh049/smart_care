  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        ...
        children: <Widget>[
          Column(children: <Widget>[const SizedBox(height: 150), Expanded(child: Container(width: MediaQuery.of(context).size.width, decoration: BoxDecoration(color: white.withOpacity(.6), borderRadius: const BorderRadius.only(topLeft: Radius.circular(35), topRight: Radius.circular(35)))))]),
          Padding(
                ...
                if (snapshot.hasData) {
                  return Column(
                    ...
                    children: <Widget>[
                      ...
                      CustomizedText(text: snapshot.data!.get("grade").isEmpty ? "Not Set" : snapshot.data!.get("grade"), fontSize: 14, color: white),
                      const SizedBox(height: 20),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        mainAxisSize: MainAxisSize.min,
                                  ...
                                  children: <Widget>[
                                    for (int index = 0; index < DateTime(DateTime.now().year, DateTime.now().month + 1, 0).day; index++)
                                      Container(
                                        ...
                                        margin: index == 0 || index == DateTime(DateTime.now().year, DateTime.now().month + 1, 0).day - 1 ? EdgeInsets.zero : const EdgeInsets.symmetric(horizontal: 8),
                                  ...
                                  children: <Widget>[
                                    for (int index = 8; index <= 18; index += 2)
                                      Container(
                                        width: 60,
                                        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                        margin: index == 10 || index == 18 ? EdgeInsets.zero : const EdgeInsets.symmetric(horizontal: 8),
                                ...
                                children: <Widget>[
                                  Expanded(
                                    child: GestureDetector(
                                      onTap: () async => await goTo(BookAppointment(id: uid, doctorName: snapshot.data!.get("name"), doctorImageUrl: snapshot.data!.get("image_url"), speciality: snapshot.data!.get("grade"))),
