      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 10),
            Padding(padding: const EdgeInsets.only(left: 12.0), child: CustomizedText(text: 'appointments'.tr, fontSize: 25, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
                  ...
                  if (snapshot.data!.docs.isNotEmpty) {
                    return StatefulBuilder(
                      ...
                      builder: (BuildContext context, void Function(void Function()) _) {
                        ...
                        data.sort((QueryDocumentSnapshot<Map<String, dynamic>> a, QueryDocumentSnapshot<Map<String, dynamic>> b) => a.get("createdAt") > b.get("createdAt"));
                        if (data.isNotEmpty) {
                          return Expanded(
                            child: ListView.separated(
                                          ...
                                          child: Column(
                                            ...
                                            children: <Widget>[
                                              Container(
                                                ...
                                                child: Center(
                                                  child: CommentTreeWidget<Tree, Tree>(
                                                    ...
                                                    contentRoot: (BuildContext context, Tree value) => Container(padding: const EdgeInsets.all(8.0), decoration: BoxDecoration(color: grey.withOpacity(.2), borderRadius: BorderRadius.circular(15)), child: Center(child: CustomizedText(text: value.text!, fontSize: 18, fontWeight: FontWeight.bold))),
                                                  ),
                                                ),
                                              ),
                                              const SizedBox(height: 10),
                                              Row(
                                                mainAxisAlignment: MainAxisAlignment.center,
                                                children: <Widget>[
                                                  GestureDetector(onTap: () => Navigator.pop(context), child: Container(padding: const EdgeInsets.all(8.0), decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), color: grey), child: CustomizedText(text: 'cancel'.tr, fontSize: 16))),
                                                  const SizedBox(width: 10),
                                                  GestureDetector(onTap: () async => await data[index].reference.delete().then((void value) => Navigator.pop(context)), child: Container(padding: const EdgeInsets.all(8.0), decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), color: blue), child: CustomizedText(text: 'closeFolderPermanently'.tr, fontSize: 16))),
                                                ],
                                              ),
                                              const SizedBox(height: 10),
                                              GestureDetector(
                              ...
                              separatorBuilder: (BuildContext context, int index) => Row(children: <Widget>[Expanded(child: Container(margin: const EdgeInsets.symmetric(vertical: 8.0), height: .2))]),
                            ),
                          );
                        } else {
                          ...
                          return GestureDetector(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: <Widget>[
                                          ...
                                          const SizedBox(height: 10),
                                          Expanded(child: Padding(padding: const EdgeInsets.all(8.0), child: SingleChildScrollView(child: CustomizedText(text: firstArtical.get("description"), fontSize: 14)))),
                                        ],
                                      ),
                                    ),
                                  ),
                            ...
                            child: Row(
                              children: <Widget>[
                                ...
                                const SizedBox(width: 10),
                                Container(padding: const EdgeInsets.symmetric(horizontal: 8.0, vertical: 4.0), decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), color: blue), height: 20, width: 20, child: const Center(child: Icon(FontAwesomeIcons.chevronRight, size: 10))),
                              ],
                            ),
                          );
                        } else {
